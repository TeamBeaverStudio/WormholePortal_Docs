...
!!! info "현재 호환성 기준"
    ...
    현재 플러그인과 문서는 Unreal Engine 5.8을 기준으로 합니다.

## 알려진 제한사항

| 영역 | 현재 제한사항 |
| --- | --- |
| 렌더링 View | SM5 이상의 단일 Primary Perspective Non-stereo Game View에서 포털 Composite를 지원합니다. |
...
| Physics Transit | 사용할 수 있는 Primitive 하나 이상과 Physics Simulation이 켜진 Static Mesh가 필요합니다. |
| Voxel Collision | Sphere, Box, Capsule, Convex Simple Collision의 Voxel Bake를 지원합니다. |
...
3. SM5 이상의 Perspective Game View를 사용합니다.
...
6. **Output Log**에서 `LogWormhole`을 필터링하고 `Reason=` 값을
   확인합니다.
...
<!-- CAPTURE SLOT I-01: Add Automatically와 Open Collision Settings가 보이는 WPPortalTrace 시작 알림. -->
...
- **Generated LUT Asset Path**가 올바른 `/Game/...` 경로여야 합니다.
...
**Generated LUT Asset Path**를 변경했다면 변경한 `/Game/...` 경로를
사용해야 합니다.
...
<!-- CAPTURE SLOT I-02: Ready, Needs Setup, Not Supported 예시가 보이는 Transit Manager. -->
...
| `Needs Setup: No simulated Static Mesh was found` | Physics Transit에는 Physics Simulation이 켜진 Static Mesh Component가 필요합니다. |
| `Needs Setup: Bake Voxel Body for the Static Mesh` | Voxel Collision을 켜고 호환되는 Voxel Data를 Bake합니다. |

Runtime 거부 로그는 다음 형태입니다.

```text
[Transit][Rejected] ... Reason=...
```

가능한 사유에는 `NotReady`, `PortalUnavailable`, `AlreadyInTransit`,
`CooldownActive`, `ActorTooLarge`, `UnsupportedActor`, `InvalidSetup`,
...
2. 원본 Static Mesh Asset을 저장해야 합니다.
3. Mesh에 Simple Collision이 있어야 합니다.
4. Collision이 Sphere, Box, Capsule 또는 Convex Shape여야 합니다.
5. **Voxel Size**가 0보다 커야 합니다.
6. **Max Voxel Count**가 `1–512` 사이여야 합니다.
...
- Material에서 Wormhole Portal Transit Clip Material Function을
  사용합니다.
...
<!-- CAPTURE SLOT I-03: LogWormhole로 필터링하고 하나의 완전한 Reason= 거부 행이 보이는 Output Log. -->
